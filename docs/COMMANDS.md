...
| `build import` | `@build base import "file" [name]` | Imports a user schematic (`.litematic`, `.schematic`, `.schem`, or Belfegor `.json`), parses it into the internal blueprint, creates a staging chest near the build site, sources every required block material in-game, deposits it into the staging chest, and builds the full structure from the sourced materials. | `@build base import "C:\builds\dirt-castle.schematic" castle` |
