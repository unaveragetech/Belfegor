...
import adris.altoclef.macros.MacroChain;
import adris.altoclef.macros.MacroRunner;
import adris.altoclef.macros.MacroStep;
...
public class AltoclefScreen extends Screen {
    ...
    private void renderTasksTab(DrawContext ctx, int mouseX, int mouseY, int y) {
        ...
        if (mod.getPlayer() != null) {
            ...
            y += 13;
            drawText(ctx, "  Cursor: "
                    + adris.altoclef.util.helpers.StorageHelper.getItemStackInCursorSlot()
    ...
    private void renderCommandsTab(DrawContext ctx, int mouseX, int mouseY, int y) {
        ...
        List<adris.altoclef.commandsystem.Command> commands = getFilteredCommands();
    ...
    private List<adris.altoclef.commandsystem.Command> getFilteredCommands() {
        if (AltoClef.getCommandExecutor() == null) return new ArrayList<>();
        String search = commandSearchInput == null
                ? "" : commandSearchInput.getText().trim().toLowerCase();
        List<adris.altoclef.commandsystem.Command> result =
                new ArrayList<>(AltoClef.getCommandExecutor().allCommands());
        ...
        result.sort(Comparator.comparing(adris.altoclef.commandsystem.Command::getName));
        return result;
    }
