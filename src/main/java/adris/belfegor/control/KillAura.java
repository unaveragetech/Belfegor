...
public class KillAura {
    ...
    public static void equipWeapon(Belfegor mod) {
    ...
    public void tickEnd(Belfegor mod) {
    ...
    private void performDelayedAttack(Belfegor mod) {
        if (!mod.getFoodChain().needsToEat() && !mod.getMLGBucketChain().isFallingOhNo(mod) &&
                mod.getMLGBucketChain().doneMLG() && !mod.getMLGBucketChain().isChorusFruiting()) {
            if (_forceHit != null) {
                attack(mod, _forceHit, true);
    ...
    private void performFastestAttack(Belfegor mod) {
        if (!mod.getFoodChain().needsToEat() && !mod.getMLGBucketChain().isFallingOhNo(mod) &&
                mod.getMLGBucketChain().doneMLG() && !mod.getMLGBucketChain().isChorusFruiting()) {
    ...
    private void attack(Belfegor mod, Entity entity) {
        attack(mod, entity, false);
    }

    private void attack(Belfegor mod, Entity entity, boolean equipSword) {
    ...
    public void startShielding(Belfegor mod) {
        if (mod.getPlayer() == null) return;
        _shielding = true;
    ...
    public void stopShielding(Belfegor mod) {
        if (_shielding) {
