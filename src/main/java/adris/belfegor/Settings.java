...
import adris.belfegor.util.BlockRange;
import adris.belfegor.util.helpers.ConfigHelper;
import adris.belfegor.util.helpers.ItemHelper;
import adris.belfegor.util.serialization.IFailableConfigFile;
...
public class Settings implements IFailableConfigFile {
    ...
    public Item[] getThrowawayItems(Belfegor mod, boolean includeProtected) {
        return throwawayItems.stream().filter(item -> includeProtected || !mod.getBehaviour().isProtected(item)).toArray(Item[]::new);
    }

    public Item[] getThrowawayItems(Belfegor mod) {
