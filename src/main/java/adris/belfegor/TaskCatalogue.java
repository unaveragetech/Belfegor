...
import adris.belfegor.tasks.container.UpgradeInSmithingTableTask;
import adris.belfegor.tasks.resources.*;
import adris.belfegor.tasks.resources.wood.*;
...
public class TaskCatalogue {
    ...
    public static ResourceTask getItemTask(String name, int count) {
        ...
        if (baseTask != null) {
            if (baseTask instanceof adris.belfegor.tasks.container.CraftInTableTask) {
                return new adris.belfegor.tasks.resources.EnsureWoodSupplyTask(baseTask, count);
            }
    ...
    private static ResourceTask getRecipeRegistryTask(String name, int count) {
        ...
        Item item = adris.belfegor.util.RecipeRegistry.getItemByName("minecraft:" + name);
        if (item == null) {
            // Try without minecraft: prefix
            item = adris.belfegor.util.RecipeRegistry.getItemByName(name);
        }
    ...
    private static ResourceTask getRecipeRegistryTask(Item item, int count) {
        adris.belfegor.util.RecipeRegistry registry = adris.belfegor.util.RecipeRegistry.getInstance();
        adris.belfegor.util.RecipeRegistry.RecipeEntry entry = registry.getRecipe(item);
        ...
        if (entry.recipe.isBig()) {
            ...
            return new adris.belfegor.tasks.container.CraftInTableTask(target);
        } else {
            // 2x2 recipe - can craft in inventory
            return new adris.belfegor.tasks.CraftInInventoryTask(target);
        }
    }
