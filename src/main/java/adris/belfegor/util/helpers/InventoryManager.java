...
public class InventoryManager {
    ...
    public InventoryManager(Belfegor mod) {
        _mod = mod;
    }
    ...
    private boolean checkSameSlotGuard(Slot slot) {
        ...
        if (slotId == _lastClickedSlotId) {
            ...
            if (_sameSlotConsecutiveCount > MAX_SAME_SLOT_CONSECUTIVE) {
                adris.belfegor.debug.DebugLogger.getInstance().log("INVMGR-STUCK",
    ...
    public boolean isShulkerBox(Item item) {
        ...
        for (Item shulker : adris.belfegor.util.helpers.ItemHelper.SHULKER_BOXES) {
