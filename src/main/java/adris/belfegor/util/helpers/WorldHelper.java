...
import adris.belfegor.mixins.ClientConnectionAccessor;
...
public interface WorldHelper {
    ...
    static boolean isSourceBlock(Belfegor mod, BlockPos pos, boolean onlyAcceptStill) {
    ...
    static boolean isSolid(Belfegor mod, BlockPos pos) {
     ...
     */
    static BlockPos getBedHead(Belfegor mod, BlockPos posWithBed) {
     ...
     */
    static BlockPos getBedFoot(Belfegor mod, BlockPos posWithBed) {
        BlockState state = mod.getWorld().getBlockState(posWithBed);
        if (state.getBlock() instanceof BedBlock) {
            Direction facing = state.get(BedBlock.FACING);
    ...
    static BlockPos getChestLeft(Belfegor mod, BlockPos posWithChest) {
    ...
    static boolean isChestBig(Belfegor mod, BlockPos posWithChest) {
        BlockState state = mod.getWorld().getBlockState(posWithChest);
        if (state.getBlock() instanceof ChestBlock) {
            ChestType type = state.get(ChestBlock.CHEST_TYPE);
    ...
    static int getGroundHeight(Belfegor mod, int x, int z) {
    ...
    static BlockPos getADesertTemple(Belfegor mod) {
    ...
    static boolean isUnopenedChest(Belfegor mod, BlockPos pos) {
        return mod.getItemStorage().getContainerAtPosition(pos).isEmpty();
    }

    static int getGroundHeight(Belfegor mod, int x, int z, Block... groundBlocks) {
    ...
    static boolean canBreak(Belfegor mod, BlockPos pos) {
    ...
    static boolean isInNetherPortal(Belfegor mod) {
    ...
    static boolean dangerousToBreakIfRightAbove(Belfegor mod, BlockPos toBreak) {
    ...
    static boolean canPlace(Belfegor mod, BlockPos pos) {
        return !mod.getExtraBaritoneSettings().shouldAvoidPlacingAt(pos)
                && canReach(mod, pos);
    }

    static boolean canReach(Belfegor mod, BlockPos pos) {
    ...
    static boolean isAir(Belfegor mod, BlockPos pos) {
    ...
    static boolean isInteractableBlock(Belfegor mod, BlockPos pos) {
    ...
    static boolean isInsidePlayer(Belfegor mod, BlockPos pos) {
        return pos.isWithinDistance(mod.getPlayer().getPos(), 2);
    }

    static Iterable<BlockPos> getBlocksTouchingPlayer(Belfegor mod) {
        return getBlocksTouchingBox(mod, mod.getPlayer().getBoundingBox());
    }

    static Iterable<BlockPos> getBlocksTouchingBox(Belfegor mod, Box box) {
    ...
    static Iterable<BlockPos> scanRegion(Belfegor mod, BlockPos start, BlockPos end) {
        return () -> new Iterator<>() {
    ...
    static Entity getSpawnerEntity(Belfegor mod, BlockPos pos) {
    ...
    static boolean isChest(Belfegor mod, BlockPos block) {
    ...
    static boolean isBlock(Belfegor mod, BlockPos pos, Block block) {
