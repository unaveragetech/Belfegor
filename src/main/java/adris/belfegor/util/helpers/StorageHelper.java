...
import adris.belfegor.ItemInfo.ToolDurabilityMapping;
import adris.belfegor.ItemInfo.ArmorSlotUtil;
...
public class StorageHelper {
    ...
    public static MiningRequirement getCurrentMiningRequirement(Belfegor mod) {
    ...
    private static boolean h(Belfegor mod, boolean inventoryOnly, Item... items) {
        if (inventoryOnly) {
    ...
    private static boolean miningRequirementMetInner(Belfegor mod, boolean inventoryOnly, MiningRequirement requirement) {
        switch (requirement) {
            case HAND:
                return true;
            case WOOD:
    ...
    public static boolean miningRequirementMet(Belfegor mod, MiningRequirement requirement) {
        return miningRequirementMetInner(mod, false, requirement);
    }

    public static boolean miningRequirementMetInventory(Belfegor mod, MiningRequirement requirement) {
        return miningRequirementMetInner(mod, true, requirement);
    }

    public static Optional<Slot> getBestToolSlot(Belfegor mod, BlockState state) {
    ...
    public static Optional<Slot> getGarbageSlot(Belfegor mod) {
     ...
     */
    public static boolean itemTargetsMet(Belfegor mod, ItemTarget... targetsToMeet) {
     ...
     */
    public static boolean itemTargetsMetInventory(Belfegor mod, ItemTarget... targetsToMeet) {
     ...
     */
    public static boolean itemTargetsMetInventoryNoCursor(Belfegor mod, ItemTarget... targetsToMeet) {
    ...
    public static boolean isArmorEquipped(Belfegor mod, Item... any) {
        for (Item item : any) {
    ...
    public static int getBuildingMaterialCount(Belfegor mod) {
    ...
    public static boolean isPlayerInventoryOpen() {
        // The player inventory is open when the client has an InventoryScreen displayed.
        // We do NOT also check player.currentScreenHandler because:
        //   1. Opening InventoryScreen is purely client-side — no packet is sent to the
        //      server, so the server-side handler may lag behind or be stale.
        //   2. After setScreen(), currentScreenHandler may still reference a previous
        //      server-opened container (e.g. crafting table) on the very next tick,
    ...
    public static boolean isArmorEquippedAll(Belfegor mod, Item... items) {
        return Arrays.stream(items).allMatch(item -> isArmorEquipped(mod, item));
    }

    public static boolean isEquipped(Belfegor mod, Item... items) {
        return ArrayUtils.contains(items, StorageHelper.getItemStackInSlot(PlayerSlot.getEquipSlot()).getItem());
    }

    public static int calculateInventoryFoodScore(Belfegor mod) {
        int result = 0;
    ...
    public static double calculateInventoryFuelCount(Belfegor mod) {
        double result = 0;
        if (!mod.getItemStorage().getItemStacksPlayerInventory(true).isEmpty()) {
            for (ItemStack stack : mod.getItemStorage().getItemStacksPlayerInventory(true)) {
    ...
    public static boolean hasRecipeMaterialsOrTarget(Belfegor mod, RecipeTarget... targets) {
    ...
    public static boolean hasCataloguedItem(Belfegor mod, String cataloguedName) {
     ...
     * are none.
     */
    public static Optional<Slot> getFilledInventorySlotInaccessibleToContainer(Belfegor mod, ItemTarget withItem) {
    ...
    public static boolean isItemInaccessibleToContainer(Belfegor mod, ItemTarget item) {
