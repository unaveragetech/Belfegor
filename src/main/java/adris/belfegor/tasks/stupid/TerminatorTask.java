...
import adris.belfegor.tasks.movement.RunAwayFromEntitiesTask;
import adris.belfegor.tasks.movement.SearchChunksExploreTask;
...
public class TerminatorTask extends Task {
    ...
    private boolean isReadyToPunk(Belfegor mod) {
    ...
    private boolean shouldPunk(Belfegor mod, PlayerEntity player) {
        if (player == null || player.isDead()) return false;
        if (player.isCreative() || player.isSpectator()) return false;
        return !mod.getButler().isUserAuthorized(player.getName().getString()) && _canTerminate.test(player);
    }

    private void tryDoFunnyMessageTo(Belfegor mod, PlayerEntity player) {
    ...
    private class ScanChunksInRadius extends SearchChunksExploreTask {
        ...
        @Override
        protected boolean isChunkWithinSearchSpace(Belfegor mod, ChunkPos pos) {
