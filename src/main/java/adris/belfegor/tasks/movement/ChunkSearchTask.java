...
abstract class ChunkSearchTask extends Task {
    ...
    // Virtual
    protected ChunkPos getBestChunk(Belfegor mod, List<ChunkPos> chunks) {
    ...
    private void searchChunkOrQueueSearch(Belfegor mod, ChunkPos pos) {
     ...
     */
    private boolean trySearchChunk(Belfegor mod, ChunkPos pos) {
        // Do NOT search later.
        if (_searchedAlready.contains(pos)) {
            return true;
        }
    ...
    protected abstract boolean isChunkPartOfSearchSpace(Belfegor mod, ChunkPos pos);
