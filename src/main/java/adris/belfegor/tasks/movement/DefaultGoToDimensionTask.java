...
import adris.belfegor.tasks.construction.compound.ConstructNetherPortalBucketTask;
...
public class DefaultGoToDimensionTask extends Task {
    ...
    private Task goToOverworldFromNetherTask(Belfegor mod) {
    ...
    private Task goToOverworldFromEndTask(Belfegor mod) {
        setDebugState("TODO: Go to center portal (at 0,0). If it doesn't exist, kill ender dragon lol");
        return null;
    }

    private Task goToNetherFromOverworldTask(Belfegor mod) {
        if (netherPortalIsClose(mod)) {
            setDebugState("Going to nether portal");
            return new EnterNetherPortalTask(Dimension.NETHER);
        }
    ...
    private Task goToEndTask(Belfegor mod) {
    ...
    private boolean netherPortalIsClose(Belfegor mod) {
