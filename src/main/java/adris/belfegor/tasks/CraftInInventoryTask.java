 ...
 */
public class CraftInInventoryTask extends ResourceTask implements adris.belfegor.tasksystem.ITaskUsesCraftingGrid {
    ...
    protected void onResourceStart(Belfegor mod) {
        ...
        if (!cursorStack.isEmpty()) {
            adris.belfegor.debug.DebugLogger.getInstance().logImmediate("CRAFT-CURSOR",
    ...
    protected Task onResourceTick(Belfegor mod) {
        ...
        if (!StorageHelper.isPlayerInventoryOpen()) {
            ...
            if (client.currentScreen == null) {
                ...
                _screenOpenCooldownTicks = 2; // Wait 2 ticks before next open attempt
            } else if (client.currentScreen instanceof InventoryScreen) {
                // Screen IS an InventoryScreen but handler check failed — wait for
                // the handler to sync instead of closing and reopening.  The retry
                // logic below will force a re-open after 20 ticks if needed.
...
// Close only after cursor recovery so a PlayerScreenHandler slot is not
    ...
    protected Task collectRecipeSubTask(Belfegor mod) {
    ...
    private void logGateState(String state) {
        ...
        adris.belfegor.debug.DebugLogger.getInstance().log("CRAFT-2X2", state + " target=" + _target);
        adris.belfegor.debug.DebugLogger.getInstance().flush();
    }
    ...
    private void openInventoryWithDiagnostics(Belfegor mod, String reason) {
        ...
        var logger = adris.belfegor.debug.DebugLogger.getInstance();
    ...
    private String inventorySnapshot(Belfegor mod) {
    ...
    private String describeHandler(Belfegor mod) {
