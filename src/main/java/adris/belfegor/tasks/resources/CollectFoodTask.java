...
import adris.belfegor.tasks.container.SmeltInSmokerTask;
...
import adris.belfegor.util.slots.SmokerSlot;
...
public class CollectFoodTask extends Task {
    ...
    private static double calculateFoodPotential(Belfegor mod) {
...
//                }
     ...
     */
    private Task pickupBlockTaskOrNull(Belfegor mod, Block blockToCheck, Item itemToGrab, Predicate<BlockPos> accept, double maxRange) {
    ...
    private Task pickupBlockTaskOrNull(Belfegor mod, Block blockToCheck, Item itemToGrab, double maxRange) {
     ...
     * Returns null if task cannot reasonably run.
     */
    private Task pickupTaskOrNull(Belfegor mod, Item itemToGrab, double maxRange) {
        Optional<ItemEntity> nearestDrop = Optional.empty();
        if (mod.getEntityTracker().itemDropped(itemToGrab)) {
            nearestDrop = mod.getEntityTracker().getClosestItemDrop(mod.getPlayer().getPos(), itemToGrab);
        }
    ...
    private Task pickupTaskOrNull(Belfegor mod, Item itemToGrab) {
