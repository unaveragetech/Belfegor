...
import adris.belfegor.eventbus.events.TaskFinishedEvent;
...
public class UserTaskChain extends SingleTaskChain {
    ...
    public void cancel(Belfegor mod) {
    ...
    public void runTask(Belfegor mod, Task task, Runnable onFinish) {
    ...
    protected void onTaskFinish(Belfegor mod) {
        ...
        if (actuallyDone) {
            ...
            if (shouldIdle) {
                Belfegor.getCommandExecutor().executeWithPrefix(mod.getModSettings().getIdleCommand());
