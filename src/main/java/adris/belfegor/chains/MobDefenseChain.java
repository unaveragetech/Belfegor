...
import adris.belfegor.ItemInfo.SwordDamageUtility;
...
public class MobDefenseChain extends SingleTaskChain {
    ...
    private static void startShielding(Belfegor mod) {
        _shielding = true;
    ...
    private void stopShielding(Belfegor mod) {
        if (_shielding) {
    ...
    private boolean escapeDragonBreath(Belfegor mod) {
    ...
    public float getPriorityInner(Belfegor mod) {
        if (!Belfegor.inGame()) {
    ...
    private BlockPos isInsideFireAndOnFire(Belfegor mod) {
    ...
    private void doForceField(Belfegor mod) {
    ...
    private CreeperEntity getClosestFusingCreeper(Belfegor mod) {
    ...
    private boolean isProjectileClose(Belfegor mod) {
                ...
                for (CachedProjectile projectile : projectiles) {
                    if (projectile.position.squaredDistanceTo(mod.getPlayer().getPos()) < 150) {
                        ...
                        if (horizontalDistanceSq < ARROW_KEEP_DISTANCE_HORIZONTAL * ARROW_KEEP_DISTANCE_HORIZONTAL && verticalDistance < ARROW_KEEP_DISTANCE_VERTICAL) {
                            if (_runAwayTask == null && BaritoneCompat.isSafeToCancel(mod.getClientBaritone().getPathingBehavior())) {
                                BaritoneCompat.requestPause(mod.getClientBaritone().getPathingBehavior());
                                {
                                    Entity owner = null;
                                    if (owner != null) {
    ...
    private Optional<Entity> getUniversallyDangerousMob(Belfegor mod) {
    ...
    private boolean isInDanger(Belfegor mod) {
    ...
    private boolean isVulnurable(Belfegor mod) {
        if (mod.getPlayer() == null) return false;
        int armor = mod.getPlayer().getArmor();
        float health = mod.getPlayer().getHealth();
